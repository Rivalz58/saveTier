...
const Tournois: React.FC<TournoisProps> = ({ user }) => {
  ...
  const [tournamentCategories, setTournamentCategories] = useState<
    Map<string, FormattedTournament[]>
  >(new Map());
  ...
  useEffect(() => {
    const loadTournaments = async () => {
      try {
        ...
        setAllTournaments(
          formattedTournaments.sort((a, b) => a.name.localeCompare(b.name)),
        );
        ...
        setError(
          "Impossible de charger les tournois. Veuillez réessayer plus tard.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const getFilteredTournaments = (): FormattedTournament[] => {
    ...
    let filtered = allTournaments.filter(
      (tournament) =>
  ...
  return (
    <div className="all-album-container">
        ...
        <div className="all-albums-section">
          <div>
            <h2 className="category-title">
              {selectedCategory || "Tous les Tournois"}
            </h2>
      ...
      <div className="create-album-prompt">
        ...
        <button className="create-album-btn" onClick={handleCreateTournament}>
