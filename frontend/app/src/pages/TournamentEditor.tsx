...
const TournamentEditor: React.FC<TournamentEditorProps> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const savedState = localStorage.getItem(
      `tournamentEditorState_${albumParam}`,
    );
    if (savedState) {
      try {
        ...
        if (
          ...
          console.log(
            "State successfully restored from localStorage for album",
            albumParam,
          );
    ...
    if (allImages.length > 0 && albumId && isInitialized) {
      ...
      try {
        localStorage.setItem(
          `tournamentEditorState_${albumId}`,
          JSON.stringify(state),
        );
        console.log(
          "Tournament state saved to localStorage at",
  ...
  const startNextRoundWithImages = (images: TournamentImage[]) => {
    ...
    console.log(
      `Round ${newCurrentRound} started with ${nextRoundImages.length} images, ${newBinome} binomes, ${newTrinome} trinomes`,
    );
  };
  ...
  const restartTournament = () => {
    if (
      !confirm(
        "Voulez-vous vraiment redémarrer le tournoi ? Toute progression non sauvegardée sera perdue.",
      )
    ) {
      return;
    }
  ...
  const handleSaveTournament = async () => {
    ...
    try {
      ...
      console.log(
        "Tournament saved successfully. Tournament ID:",
        tournamentId,
      );
  ...
  const isFinalRound = () => {
    ...
    const maxScore = scoreMax(allImages);
    const imagesWithMaxScore = allImages.filter(
      (img) => img.score === maxScore,
    );
    return (
      (imagesWithMaxScore.length === 2 || imagesWithMaxScore.length === 3) &&
  ...
  return (
    <div className="tournament-editor">
        ...
        <div className="info-modal-overlay">
          <div className="info-modal-content">
            ...
            <div className="info-content">
              ...
              <ul>
                <li>
                  <strong>Tournament Format:</strong> Images compete in
                  head-to-head matches
                </li>
                <li>
                  <strong>Selection:</strong> Click on your favorite image in
                  each match
                </li>
                <li>
                  <strong>Rounds:</strong> Continue through rounds until a
                  winner emerges
                </li>
                <li>
                  <strong>Customization:</strong> Click on the edit button to
                  change tournament details
                </li>
                <li>
                  <strong>Results:</strong> View final rankings when the
                  tournament is complete
                </li>
              </ul>
              <p>
                Remember to fill in tournament information and save your
                tournament!
              </p>
            </div>
      ...
      <div
        className={`tournament-main-content ${showSidebar ? "with-sidebar" : ""}`}
      >
        {tournamentFinished ? (
