...
export const getUserAlbums = async (userID: string): Promise<AlbumItem[]> => {
  try {
    ...
    const userAlbums = response.data.data
      ...
      .filter(
        (album: any) => album.author && album.author.id.toString() === userID,
      )
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .map((album: any) => ({
  ...
  try {
    ...
    const userTierlists = response.data.data
      ...
      .map((tierlist: any) => ({
        ...
        image:
          tierlist.album &&
          tierlist.album.image &&
          tierlist.album.image.length > 0
...
export const getUserTournaments = async (
  ...
  try {
    ...
    const userTournaments = response.data.data
      ...
      .filter(
        (tournament: any) =>
          tournament.author && tournament.author.id.toString() === userID,
      )
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .map((tournament: any) => ({
        ...
        image:
          tournament.album &&
          tournament.album.image &&
          tournament.album.image.length > 0
        ...
        categories:
          tournament.album && tournament.album.categories
            ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const getUserRankings = async (
  ...
  try {
    ...
    const userRankings = response.data.data
      ...
      .filter(
        (ranking: any) =>
          ranking.author && ranking.author.id.toString() === userID,
      )
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .map((ranking: any) => ({
        ...
        name: ranking.name,
        image:
          ranking.album && ranking.album.image && ranking.album.image.length > 0
        ...
        categories:
          ranking.album && ranking.album.categories
            ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
