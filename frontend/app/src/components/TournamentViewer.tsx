...
const closeImageDetailsModal = () => {
  ...
  const [albumInfo, setAlbumInfo] = useState<{categories: string[], imagePath: string}>({
    categories: [],
    imagePath: '/default-image.jpg'
  });
  const [rankedImages, setRankedImages] = useState<{[round: number]: TournamentImage[]}>({});
  ...
  useEffect(() => {
    const fetchTournamentData = async () => {
      try {
        ...
        try {
          const albumInformation = await getAlbumInfoForContent(tournament.album.id);
        ...
        setError("Unable to load tournament data. It might not exist or you may not have permission to view it.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const processImageRanking = (tournament: TournamentData) => {
    if (!tournament || !tournament.tournamentImage || tournament.tournamentImage.length === 0) {
      return;
    }
    ...
    return (
      <div className="tournament-viewer">
        <div className="error-message">
          ...
          <p>The tournament you are looking for does not exist or has been removed.</p>
          <button onClick={handleBack} className="back-button">Back</button>
        </div>
      </div>
    );
    ...
    <div className="tournament-viewer">
      ...
      <div className="tournament-info-panel">
        <div className="tournament-metadata">
          <p><strong>Album:</strong> {tournamentData.album.name}</p>
          <p><strong>Created by:</strong> {tournamentData.author.username}</p>
          <p><strong>Creation date:</strong> {new Date(tournamentData.createdAt).toLocaleDateString()}</p>
          ...
          {albumInfo.categories.length > 0 && (
            <div className="tournament-categories">
              <p><strong>Categories:</strong> {albumInfo.categories.join(', ')}</p>
            </div>
          )}
        </div>
      </div>
