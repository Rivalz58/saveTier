  ...
  useEffect(() => {
    const fetchTierlistData = async () => {
      ...
      try {
        ...
        const response = await tierlistService.getTierlistWithDetails(parseInt(id));
        ...
        for (const line of response.lines) {
          ...
          console.log(`Analyse de la ligne ${line.id}, placement: ${line.placement}, label: ${line.label}`);
        ...
        const sortedLines = normalTiers.sort((a, b) => a.placement - b.placement);
        setTierLines(sortedLines);
        
        // Définir les images non classées si elles existent
        if (unclassifiedTier && unclassifiedTier.images) {
          console.log("Images non classées détectées:", unclassifiedTier.images.length);
        ...
        try {
          const albumResponse = await tierlistService.getAlbumImages(response.tierlist.id_album.toString());
          if (albumResponse && albumResponse.length > 0) {
            ...
            const albumInfo = await fetch(`/api/album/${response.tierlist.id_album}`).then(res => res.json());
        ...
        setError("Impossible de charger la tierlist. Elle n'existe pas ou vous n'avez pas les permissions nécessaires.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="tierlist-editor">
      ...
      <div className="tierlist-info-panel">
        <div className="tierlist-metadata">
          ...
          <p><strong>Créateur:</strong> {authorName}</p>
          {tierlistDescription && (
